...
class DeepZoomGenerator(object):
    ...
    def __init__(self, osr, tile_size=254, overlap=1, limit_bounds=False):
        ...
        raw_downsamples = self._osr.level_downsamples
        if raw_downsamples and max(raw_downsamples) <= 1.0:
            self._l0_l_downsamples = tuple(1.0 / d if d > 0 else float('inf') for d in raw_downsamples)
        else:
            self._l0_l_downsamples = raw_downsamples
    ...
    def get_tile(self, level, address):
        ...
        # SDPC read_region returns RGB, convert to RGBA for compositing
        if tile.mode == 'RGB':
...
def main():
    sdpc_file_path = "/home/username/Development/DATA/TEST_DATA/test001.sdpc"
    slide = SdpcSlide(sdpc_file_path)
